...
(function () {
  ...
  function mount() {
    ...
    root.innerHTML =
      ...
      ".bar{background:#1c1d1f;color:#fff;padding:8px 14px;display:flex;gap:10px;align-items:center}" +
      ".bar b{font-weight:650}.bar .sp{flex:1}" +
      "button{font:inherit;border:1px solid #3a3b3e;background:#2a2b2e;color:#fff;border-radius:6px;padding:4px 10px;cursor:pointer}" +
      "button.go{background:#2f9e44;border-color:#2f9e44}" +
      ".panel{background:#fff;color:#1c1d1f;max-height:52vh;overflow:auto;padding:12px 14px;border-top:1px solid #e4e4e2}" +
      ".panel[hidden]{display:none}.el{font-family:ui-monospace,Menlo,monospace;font-weight:700}" +
      ".loc{color:#6b6f76;font-size:11px;margin-bottom:8px}" +
      ".br{display:flex;gap:8px;align-items:baseline;padding:4px 0}" +
      ".br input[type=text]{font:12px ui-monospace,monospace;border:1px solid #e4e4e2;border-radius:5px;padding:3px 6px;width:220px}" +
      ".fx{color:#6b6f76;font-size:12px}.hi{outline:2px solid #2f6fed!important;outline-offset:1px}</style>" +
      "<div class=bar><b>Precedence</b><span class=sp id=msg>click an element to track it</span>" +
      "<span id=count></span><button class=go id=send>send to wizard</button></div>" +
  ...
  function onHover(e) {
    if (root.host.contains(e.target)) return;
