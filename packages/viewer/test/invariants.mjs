/**
 * @precedence-dev/viewer invariants: the local picker server (`servePlan`) and
 * the in-page picker agent's pure logic (`browser/agent.js`).
 */
import * as path from "node:path";
import { fileURLToPath, pathToFileURL } from "node:url";

const here = path.dirname(fileURLToPath(import.meta.url));
const { servePlan } = await import(pathToFileURL(path.resolve(here, "../dist/index.js")).href);
...
/* ---- servePlan: the local receiver the wizard drives ---- */
{
  let url;
  ...
  check("servePlan: GET / is not served — there is no standalone page",
